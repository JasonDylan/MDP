...
def func4(S, L_server):
    L_levels = 5
    N_1 = [sum(1 for l_m, (i_m, w_m) in zip(L_server, S[1]) if l_m == j and w_m != 0) for j in range(1, L_levels+1)]
...
S = state
L_server = [1,1,1,2,3,4,5,2,3,5, 1,1,1,2,3,4,5,2,3,5,1,1,1,2,3,4,5,2,3,5,1,1,1,2,3,4,5,2,3,5,]
...
def lp_aggreg(S, Z_cluster_num, X, M_servers, I_citys, L_levels):
    ...
    for z_cluster in range(num_cluster):
        e_z = pulp.lpSum([1 for m_server in range(M_servers) if S[1][m_server][0] in cluster[z_cluster]])
        if e_z == 0:
            g[z_cluster] = 0  
        elif e_z <= barM / num_cluster:
            g[z_cluster] = 1  
    ...
    N=[[pulp.LpVariable(f"k_{i}",cat='Integer') for i in range(num_cluster)]    for j in range(L_levels)]
    
    for z_cluster in range(num_cluster):
        for l in range(L_levels):
            N[z_cluster][l] = min(X, pulp.lpSum([S[0][i-1][l] for i in cluster[z_cluster]]))
...
def func3_transfer(S, L_server, H_home_of_server, r1, c1, c2):
    ...
    for i in range(I_citys):
        for l in range(L_max):
            n_next[i][l] = n_il[i][l] - pulp.lpSum(y[m,i,l+1] for m in range(M_servers))            +func7(T, x_max_task_num, lambda_il)
    for m in range(M_servers):
    ...
    prob += pulp.lpSum(
        r1[l] * y[m, i, l] - c1[servers_info[m][0]-1][i] * y[m, i, l]
                       ...
                       for l in range(0, L_max+1))\
                        - c2 * pulp.lpSum(n_il[i][l-1] - pulp.lpSum(y[m, i, l] for m in range(M_servers)) for i in range(I_citys)
                       for l in range(1, L_max+1))+V(S_next)
