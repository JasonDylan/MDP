...
class Func8:
    ...
    def __init__(self):
        ...
        self.S_init_per_iter = [None] * self.max_iter
    ...
    def generate_iter_S_inits(self):
    ...
    def calc_reward(self, S, Y_allocation):
        """
        计算即时奖励。

        参数:
        S (tuple): 当前状态。
        Y_allocation (list): 决策,包含每个服务员的位置和等级。
        ...
        # 计算服务员完成任务的收益
        reward_service = sum(self.r1_reward[l] for m, i, l in Y_allocation if l > 0)

        # 计算服务员的城市距离成本
        cost_distance = sum(
            self.c1_city_cost[servers_info[m][0] - 1][i - 1] for m, i, l in Y_allocation
        )

        # 计算未完成任务的惩罚成本
        cost_unfinished = self.c2 * sum(
            n_il[i][l - 1]
            - sum(1 for m, i_, l_ in Y_allocation if i_ == i + 1 and l_ == l)
            for i in range(I_citys)
            for l in range(1, L_levels + 1)
        )

        # 总即时奖励
        reward = reward_service - cost_distance - cost_unfinished
    ...
    def calc_total_reward_for_S_init_by_nearest(self, S_t):
        """
        最近分配策略下的总收益计算。
        ...
        for t in range(self.T):
            Y_allocation = self.nearest_allocation(S_current)
    ...
    def nearest_allocation(self, S):
        ...
        # 计算每个服务员与每个任务之间的距离
        distances = {
            (m, i, l): self.c1_city_cost[servers_info[m][0] - 1][i]
            for m in range(M_servers)
            for i in range(I_citys)
            for l in range(1, L_levels + 1)
        }
        ...
        for m in range(M_servers):
            ...
            if wm == 0:
                ...
                available_tasks = [
                    ...
                    for i in range(I_citys)
                    for l in range(1, L_levels + 1)
                    if n_il[i][l - 1] > 0 and l >= self.mathscr_L[m]
                ]
                ...
                if available_tasks:
                    ...
                    allocation[m] = (m, nearest_task[0] + 1, nearest_task[1])
    ...
    def func12(self):
        ...
        for i in range(self.max_iter):
            print(f"-------------iters:{i=}--------------")
            S_init = self.S_init_per_iter[i]
            reward = self.calc_total_reward_for_S_init_by_nearest(S_init)
            S_str = str(S_init)
            last_reward = total_reward_for_S0.get(S_str, 0)
            if last_reward < reward:
                total_reward_for_S0[S_str] = reward
    ...
    def calc_total_reward_for_S_init_by_single_stage_opt(self, S_t):
        """
        单阶段最优分配策略下的总收益计算。
        ...
        for t in range(self.T):
            Y_allocation, R_t = self.func3_transfer(
                S_t=S_current,
                t=t,
    ...
    def func13(self):
        ...
        for i in range(self.max_iter):
            print(f"-------------iters:{i=}--------------")
            S_init = self.S_init_per_iter[i]
            reward = self.calc_total_reward_for_S_init_by_single_stage_opt(S_init)
            S_str = str(S_init)
            last_reward = total_reward_for_S0.get(S_str, 0)
            if last_reward < reward:
                total_reward_for_S0[S_str] = reward
...
func8_instance.generate_iter_S_inits()
...
state_value_rnd = func8_instance.func11()
state_value_nearest = func8_instance.func12()
state_value_single_stage = func8_instance.func12()