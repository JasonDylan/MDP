...
def save_to_csv(result, T_values, Z_values, S_n, file_name="./data/results.csv"):
    ...
    """
    headers = [""] + [
        f"T={T},Z_cluster_num={Z_cluster_num}"
        for T in T_values
        for Z_cluster_num in Z_values
    ]
...
class TaskRunner:
    ...
    def run_benchmark_count_big_1(self):
        ...
        for T_idx, T in enumerate(T_values):
            for Z_idx, Z_cluster_num in enumerate(Z_values):
                ...
                selected_elements = [
                    self.problem.init_S_J[i] for i in count_s_count_bigger_list
                ]
                for S_idx, s in enumerate(selected_elements):
                    ...
                    logging.info(
                        f"{(S_idx,1,T_idx,Z_idx)=} {result[S_idx][1][T_idx][Z_idx]=}"
                    )
                    logging.info(
                        f"{(S_idx,2,T_idx,Z_idx)=} {result[S_idx][2][T_idx][Z_idx]=}"
                    )
                    logging.info(
                        f"{(S_idx,3,T_idx,Z_idx)=} {result[S_idx][3][T_idx][Z_idx]=}"
                    )
    ...
    def run(self, T, Z_cluster_num, J, S_n=5, solution_types=4, Process_id=0) -> SValue:
        ...
        try:
            ...
            logging.info(
                f"{(T,Z_cluster_num)=} s_value memory usage: {s_value_memory} bytes"
            )
        finally:
            # 获取文件夹路径
            folder_path = os.path.dirname(
                f"./data/save_params/s_value_{T}_{Z_cluster_num}_{J}.pkl"
            )
            ...
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)
            with open(
                f"./data/save_params/s_value_{T}_{Z_cluster_num}_{J}.pkl", "wb"
            ) as file:
...
def main():
    ...
    task_args = [
        ...
        for Process_id, (T, Z_cluster_num) in enumerate(
            (T, Z) for T in T_values for Z in Z_values
        )
    ]
    ...
    try:
        ...
        for i, (T, Z_cluster_num, _, _, _, _) in enumerate(task_args):
            ...
            print(
                f"[DEBUG] result[:, :, T_idx, Z_idx] shape: {result[:, :, T_idx, Z_idx].shape}"
            )
